...
#define  DEFAULT_WRITING_APP             "GStreamer My Matroska muxer"
...
gst_matroska_mux_get_type (void)
{
  ...
  if (object_type == 0) {
    ...
    object_type = g_type_register_static (GST_TYPE_ELEMENT,
        "MyGstMatroskaMux", &object_info, (GTypeFlags) 0);
...
gst_matroska_mux_class_init (GstMatroskaMuxClass * klass)
{
  ...
  gst_element_class_set_static_metadata (gstelement_class, "My Matroska muxer",
      "Codec/Muxer",
  ...
  signals[SIGNAL_RECORD_INFO] =
	  g_signal_new("record-info", G_TYPE_FROM_CLASS(klass),
	  G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_generic, G_TYPE_NONE, 1, G_TYPE_POINTER);
}
...
gst_matroska_mux_init (GstMatroskaMux * mux, gpointer g_class)
{
  ...
  mux->record_info = g_new0(RecordInfo,1);
...
gst_matroska_mux_reset (GstElement * element)
{
  ...
  mux->record_info->recordtime = (GTimeVal){ 0, 0 };
  mux->record_info->startframets = mux->record_info->endframets =0;
}
...
gst_matroska_mux_start (GstMatroskaMux * mux)
{
  ...
  GTimeVal *time = &(mux->record_info->recordtime);//{ 0, 0 };
  gchar s_id[32];
#if 0
  GstToc *toc;
#endif
  ...
  gst_ebml_write_utf8 (ebml, GST_MATROSKA_ID_MUXINGAPP,"GStreamer mymatroskamux");
...
gst_matroska_mux_finish (GstMatroskaMux * mux)
{
  ...
  for (collected = mux->collect->data; collected;
	...
	/*获取录像文件的第一帧和最后一帧的TS*/
	if (((mux->record_info->startframets > 0) && (collect_pad->start_ts < mux->record_info->startframets)) 
		|| (0 == mux->record_info->startframets))
	{
		mux->record_info->startframets = collect_pad->start_ts;
	}
	if (((mux->record_info->endframets > 0) && (collect_pad->end_ts > mux->record_info->endframets))
		|| (0 == mux->record_info->endframets))
	{
		mux->record_info->endframets = collect_pad->end_ts;
	}
  ...
  /* 发送record-info信号 */
  g_signal_emit(mux, signals[SIGNAL_RECORD_INFO], 0, mux->record_info);
}
    ...
    GstBuffer * buf, gpointer user_data)
{
  ...
  if (GST_CLOCK_TIME_IS_VALID (buffer_timestamp)) {
	...
	/*这一段注释掉，主要是由于default_duration是根据CAPS的帧率算出的，与实际发送的buf中的pts有差距*/
...
gboolean
register_mymatroskamux(GstPlugin * plugin)
{
	GST_DEBUG_CATEGORY_INIT(matroskamux_debug, "mymatroskamux", 0, "recorder media File Muxing Sink");

	return gst_element_register(plugin, "mymatroskamux", GST_RANK_NONE, GST_TYPE_MATROSKA_MUX);
}
